...
const MyNotes = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const { data: response } = await axios.get(`/notes/notes/id`);
        setData(response);
      } catch (error) {
  ...
  return (
    <div className="notesPage">
      ...
      <div className="myNotespage">
        {data.map((note, id) => (
          <div className="myNotes-container" key={note.id}>
            {console.log("id:", id)}
            ...
            <div className="card-text">
              ...
              <Link to={`/single-note/${id}`} className="link-button">
                <button
                  ...
                  onMouseOver={mouseOverChevron}
                  onMouseLeave={mouseOverChevron}
                >
                  See note{" "}
                  <FaChevronRight
                    className={hoverButton ? "svg-fill-white" : "null"}
                  />
                </button>
              </Link>
            </div>
