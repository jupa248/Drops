export const EditIcon = () => {
  return (
    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M13 21H21"
        stroke="#323232"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M20.0651 7.39423L7.09967 20.4114C6.72438 20.7882 6.21446 21 5.68265 21H4.00383C3.44943 21 3 20.5466 3 19.9922V18.2987C3 17.7696 3.20962 17.2621 3.58297 16.8873L16.5517 3.86681C19.5632 1.34721 22.5747 4.87462 20.0651 7.39423Z"
        stroke="#323232"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M15.3096 5.30981L18.7273 8.72755"
        stroke="#323232"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        opacity="0.1"
        d="M18.556 8.90942L7.09967 20.4114C6.72438 20.7882 6.21446 21 5.68265 21H4.00383C3.44943 21 3 20.5466 3 19.9922V18.2987C3 17.7696 3.20962 17.2621 3.58297 16.8873L15.0647 5.35974C15.0742 5.4062 15.0969 5.45049 15.1329 5.48653L18.5506 8.90426C18.5524 8.90601 18.5542 8.90773 18.556 8.90942Z"
        fill="#323232"
      />
    </svg>
  );
};

export const BarrelIcon = () => {
  return (
    <svg
      // height="800px"
      // width="800px"
      version="1.1"
      id="Capa_1"
      xmlns="http://www.w3.org/2000/svg"
      // xmlns:xlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 395.67 395.67"
      // xml:space="preserve"
    >
      <g id="XMLID_29_">
        <g>
          <path
            fill="#666666"
            d="M147.978,192.754c10.11,0,18.34,8.23,18.34,18.35c0,10.11-8.23,18.34-18.34,18.34
       c-10.12,0-18.35-8.23-18.35-18.34C129.628,200.984,137.858,192.754,147.978,192.754z"
          />
          <polygon
            fill="#B3B3B3"
            points="353.008,320.384 344.118,342.064 51.548,342.064 42.668,320.384 		"
          />
          <polygon
            fill="#B3B3B3"
            points="344.118,53.604 353.008,75.284 42.668,75.284 51.548,53.604 		"
          />
          <path
            fill="#E0974F"
            d="M105.218,380.424h-12.01c-6.68-7.12-12.96-14.95-18.81-23.36h18.37
       C96.638,365.344,100.798,373.154,105.218,380.424z"
          />
          <path
            fill="#E0974F"
            d="M156.038,380.424h-33.02c-4.82-7.13-9.36-14.95-13.59-23.36h41.43
       C152.468,365.354,154.198,373.164,156.038,380.424z"
          />
          <path
            fill="#E0974F"
            d="M166.148,357.064h63.38c-1.67,8.37-3.47,16.18-5.38,23.36h-52.62
       C169.618,373.244,167.818,365.434,166.148,357.064z"
          />
          <path
            fill="#E0974F"
            d="M244.808,357.064h41.44c-4.22,8.39-8.76,16.21-13.59,23.36h-33.02
       C241.478,373.164,243.208,365.354,244.808,357.064z"
          />
          <path
            fill="#E0974F"
            d="M302.908,357.064h18.38c-5.84,8.41-12.13,16.24-18.81,23.36h-12.02
       C294.888,373.134,299.048,365.334,302.908,357.064z"
          />
          <path
            fill="#E0974F"
            d="M321.288,38.604h-18.38c-3.86-8.27-8.02-16.07-12.45-23.35h12.02
       C309.158,22.374,315.448,30.194,321.288,38.604z"
          />
          <path
            fill="#E0974F"
            d="M286.248,38.604h-41.44c-1.6-8.29-3.33-16.1-5.17-23.35h33.02
       C277.488,22.394,282.028,30.214,286.248,38.604z"
          />
          <path
            fill="#E0974F"
            d="M229.528,38.604h-63.38c1.67-8.36,3.47-16.18,5.38-23.35h52.62
       C226.058,22.424,227.858,30.244,229.528,38.604z"
          />
          <path
            fill="#E0974F"
            d="M123.018,15.254h33.02c-1.84,7.25-3.57,15.06-5.18,23.35h-41.43
       C113.658,30.194,118.198,22.374,123.018,15.254z"
          />
          <path
            fill="#E0974F"
            d="M93.208,15.254h12.01c-4.42,7.27-8.58,15.07-12.45,23.35h-18.37
       C80.248,30.194,86.528,22.364,93.208,15.254z"
          />
          <path
            fill="#E0974F"
            d="M348.448,90.284c12.73,33,19.46,70.06,19.46,107.55s-6.73,74.55-19.46,107.55h-27
       c8.88-33.29,13.56-70.23,13.56-107.55s-4.68-74.26-13.56-107.55H348.448z"
          />
          <path
            fill="#E0974F"
            d="M305.908,90.284c9.23,33.02,14.1,70.06,14.1,107.55c0,37.48-4.87,74.53-14.1,107.55h-53.44
       c3.65-33.17,5.58-70.14,5.58-107.55s-1.93-74.38-5.58-107.55H305.908z"
          />
          <path
            fill="#E0974F"
            d="M237.378,90.284c3.72,33.04,5.67,70.06,5.67,107.55c0,37.48-1.95,74.51-5.67,107.55h-79.08
       c-2.19-19.43-3.75-40.02-4.68-61.43c15.71-2.68,27.7-16.39,27.7-32.85c0-16.75-12.41-30.64-28.51-32.99
       c0.55-30.93,2.39-60.41,5.48-87.83H237.378z"
          />
          <path
            fill="#E0974F"
            d="M89.768,90.284h53.43c-3.08,27.86-4.9,57.75-5.41,89.07c-13.42,4.32-23.16,16.92-23.16,31.75
       c0,15.12,10.12,27.91,23.94,31.98c0.91,21.67,2.45,42.57,4.63,62.3h-53.43c-9.23-33.02-14.11-70.07-14.11-107.55
       C75.658,160.344,80.538,123.304,89.768,90.284z"
          />
          <path
            fill="#E0974F"
            d="M47.218,90.284h27.01c-8.88,33.29-13.57,70.23-13.57,107.55s4.69,74.26,13.57,107.55h-27.01
       c-12.73-33-19.46-70.06-19.46-107.55S34.488,123.284,47.218,90.284z"
          />
          <path
            fill="#4D4D4D"
            d="M364.458,90.274c12.08,33.33,18.45,70.26,18.45,107.56s-6.37,74.24-18.45,107.57
       c2.39,0.08,4.61,1.3,5.95,3.3c1.39,2.08,1.66,4.71,0.71,7.02l-15.02,36.69c-1.16,2.81-3.9,4.65-6.94,4.65h-9.88
       c-8.43,13.32-17.9,25.45-28.21,36.08c-1.41,1.45-3.35,2.28-5.38,2.28H89.998c-2.03,0-3.98-0.83-5.39-2.28
       c-10.3-10.63-19.77-22.76-28.21-36.08h-9.88c-3.04,0-5.79-1.84-6.94-4.65l-15.03-36.69c-0.95-2.31-0.68-4.94,0.71-7.02
       c1.35-2,3.56-3.21,5.96-3.3c-12.08-33.33-18.46-70.27-18.46-107.57s6.38-74.23,18.46-107.56c-2.4-0.09-4.61-1.31-5.96-3.31
       c-1.39-2.07-1.66-4.71-0.71-7.02l15.03-36.68c1.15-2.82,3.9-4.66,6.94-4.66h9.88c8.44-13.32,17.91-25.45,28.21-36.07
       c1.41-1.46,3.36-2.28,5.39-2.28h215.69c2.03,0,3.97,0.82,5.38,2.27c10.31,10.63,19.78,22.76,28.21,36.08h9.88
       c3.04,0,5.78,1.84,6.94,4.66l15.02,36.68c0.95,2.31,0.68,4.95-0.71,7.02C369.068,88.964,366.848,90.184,364.458,90.274z
        M367.908,197.834c0-37.49-6.73-74.55-19.46-107.55h-27c8.88,33.29,13.56,70.23,13.56,107.55s-4.68,74.26-13.56,107.55h27
       C361.178,272.384,367.908,235.324,367.908,197.834z M353.008,75.284l-8.89-21.68H51.548l-8.88,21.68H353.008z M344.118,342.064
       l8.89-21.68H42.668l8.88,21.68H344.118z M302.908,38.604h18.38c-5.84-8.41-12.13-16.23-18.81-23.35h-12.02
       C294.888,22.534,299.048,30.334,302.908,38.604z M321.288,357.064h-18.38c-3.86,8.27-8.02,16.07-12.45,23.36h12.02
       C309.158,373.304,315.448,365.474,321.288,357.064z M320.008,197.834c0-37.49-4.87-74.53-14.1-107.55h-53.44
       c3.65,33.17,5.58,70.14,5.58,107.55s-1.93,74.38-5.58,107.55h53.44C315.138,272.364,320.008,235.314,320.008,197.834z
        M244.808,38.604h41.44c-4.22-8.39-8.76-16.21-13.59-23.35h-33.02C241.478,22.504,243.208,30.314,244.808,38.604z
        M286.248,357.064h-41.44c-1.6,8.29-3.33,16.1-5.17,23.36h33.02C277.488,373.274,282.028,365.454,286.248,357.064z
        M243.048,197.834c0-37.49-1.95-74.51-5.67-107.55h-79.09c-3.09,27.42-4.93,56.9-5.48,87.83c16.1,2.35,28.51,16.24,28.51,32.99
       c0,16.46-11.99,30.17-27.7,32.85c0.93,21.41,2.49,42,4.68,61.43h79.08C241.098,272.344,243.048,235.314,243.048,197.834z
        M166.148,38.604h63.38c-1.67-8.36-3.47-16.18-5.38-23.35h-52.62C169.618,22.424,167.818,30.244,166.148,38.604z M229.528,357.064
       h-63.38c1.67,8.37,3.47,16.18,5.38,23.36h52.62C226.058,373.244,227.858,365.434,229.528,357.064z M166.318,211.104
       c0-10.12-8.23-18.35-18.34-18.35c-10.12,0-18.35,8.23-18.35,18.35c0,10.11,8.23,18.34,18.35,18.34
       C158.088,229.444,166.318,221.214,166.318,211.104z M156.038,15.254h-33.02c-4.82,7.12-9.36,14.94-13.59,23.35h41.43
       C152.468,30.314,154.198,22.504,156.038,15.254z M123.018,380.424h33.02c-1.84-7.26-3.57-15.07-5.18-23.36h-41.43
       C113.658,365.474,118.198,373.294,123.018,380.424z M143.198,90.284h-53.43c-9.23,33.02-14.11,70.06-14.11,107.55
       c0,37.48,4.88,74.53,14.11,107.55h53.43c-2.18-19.73-3.72-40.63-4.63-62.3c-13.82-4.07-23.94-16.86-23.94-31.98
       c0-14.83,9.74-27.43,23.16-31.75C138.298,148.034,140.118,118.144,143.198,90.284z M105.218,15.254h-12.01
       c-6.68,7.11-12.96,14.94-18.81,23.35h18.37C96.638,30.324,100.798,22.524,105.218,15.254z M93.208,380.424h12.01
       c-4.42-7.27-8.58-15.08-12.45-23.36h-18.37C80.248,365.474,86.528,373.304,93.208,380.424z M74.228,90.284h-27.01
       c-12.73,33-19.46,70.06-19.46,107.55s6.73,74.55,19.46,107.55h27.01c-8.88-33.29-13.57-70.23-13.57-107.55
       S65.348,123.574,74.228,90.284z"
          />
        </g>
        <g></g>
      </g>
    </svg>
  );
};

export const EyeIcon = () => {
  return (
    <svg
      version="1.0"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      //xmlns:xlink="http://www.w3.org/1999/xlink"
      width="3.5rem"
      height="3.5rem"
      viewBox="0 0 64 64"
      //enable-background="new 0 0 64 64"
      //xml:space="preserve"
    >
      <g>
        <path
          fill="#F9EBB2"
          d="M32,54.004c-19.686,0-28.677-19.123-29.917-22.001C3.321,29.121,12.288,10.004,32,10.004
		c19.686,0,28.677,19.123,29.917,22.001C60.679,34.887,51.712,54.004,32,54.004z"
        />
        <circle fill="#45AAB8" cx="32" cy="32.004" r="14" />
        <circle fill="#506C7F" cx="32" cy="32.008" r="6" />
        <g>
          <path
            fill="#394240"
            d="M63.934,31.645c-0.015-0.037-0.256-0.646-0.74-1.648C60.597,24.627,51.02,8.004,32,8.004
			c-22.568,0-31.842,23.404-31.934,23.641c-0.089,0.231-0.089,0.487,0,0.719C0.158,32.6,9.432,56.004,32,56.004
			c19.01,0,28.587-16.605,31.189-21.983c0.486-1.007,0.729-1.62,0.744-1.657C64.022,32.132,64.022,31.876,63.934,31.645z M32,54.004
			c-19.686,0-28.677-19.123-29.917-22.001C3.321,29.121,12.288,10.004,32,10.004c19.686,0,28.677,19.123,29.917,22.001
			C60.679,34.887,51.712,54.004,32,54.004z"
          />
          <path
            fill="#394240"
            d="M32,16.008c-8.837,0-16,7.163-16,16s7.163,16,16,16s16-7.163,16-16S40.837,16.008,32,16.008z M32,46.008
			c-7.732,0-14-6.268-14-14s6.268-14,14-14s14,6.268,14,14S39.732,46.008,32,46.008z"
          />
          <path
            fill="#394240"
            d="M32,24.008c-4.418,0-8,3.582-8,8s3.582,8,8,8s8-3.582,8-8S36.418,24.008,32,24.008z M32,38.008
			c-3.313,0-6-2.687-6-6s2.687-6,6-6s6,2.687,6,6S35.313,38.008,32,38.008z"
          />
          <path
            fill="#394240"
            d="M32,28.004c-0.553,0-1,0.447-1,1s0.447,1,1,1c1.104,0,2,0.896,2,2c0,0.553,0.447,1,1,1s1-0.447,1-1
			C36,29.795,34.209,28.004,32,28.004z"
          />
        </g>
      </g>
    </svg>
  );
};

export const NoseIcon = () => {
  return (
    <svg
      width="3.5rem"
      height="3.5rem"
      viewBox="0 0 64 64"
      xmlns="http://www.w3.org/2000/svg"
      //xmlns:xlink="http://www.w3.org/1999/xlink"
      aria-hidden="true"
      role="img"
      //class="iconify iconify--emojione"
      preserveAspectRatio="xMidYMid meet"
    >
      <g fill="#ffdd67">
        <ellipse cx="16.9" cy="48.6" rx="9.9" ry="9.5"></ellipse>

        <ellipse cx="47.1" cy="48.6" rx="9.9" ry="9.5"></ellipse>
      </g>

      <g fill="#ffc95c">
        <path d="M47.1 49.4c-3.8 0-6.9 2.1-6.9 4.7c0 .6.2 1.2.5 1.8c1.7 1.4 3.9 2.3 6.4 2.3c2.4 0 4.7-.9 6.4-2.3c.3-.5.5-1.1.5-1.8c0-2.6-3-4.7-6.9-4.7"></path>

        <path d="M16.9 49.4c3.8 0 6.9 2.1 6.9 4.7c0 .6-.2 1.2-.5 1.8c-1.7 1.4-3.9 2.3-6.4 2.3c-2.4 0-4.7-.9-6.4-2.3c-.3-.5-.5-1.1-.5-1.8c0-2.6 3-4.7 6.9-4.7"></path>

        <path d="M32 53.3c-3.1 0-7-.9-12.4-2l1.1 6.1C23.6 60.2 27.6 62 32 62s8.4-1.8 11.3-4.6l1.1-6.1c-5.4 1.1-9.3 2-12.4 2"></path>
      </g>

      <g fill="#ad6c3f">
        <path d="M17.6 53.2c3.7 1.1 4.7 5.6 4.7 5.6s-2.5-1.9-5.4-2.5c-6.7-1.4-3.8-4.6.7-3.1"></path>

        <path d="M46.4 53.2c-3.7 1.1-4.7 5.6-4.7 5.6s2.5-1.9 5.4-2.5c6.8-1.4 3.8-4.6-.7-3.1"></path>
      </g>

      <path
        d="M43 21.2C43 5 38.1 2 32 2S20.9 5 20.9 21.2c0 4.7-3.7 18.4-3.7 24c0 8.3 6.6 8.6 14.8 8.6c8.2 0 14.8-.3 14.8-8.6c0-5.6-3.8-19.3-3.8-24"
        fill="#ffdd67"
      ></path>
    </svg>
  );
};

export const MouthIcon = () => {
  return (
    <svg
      width="3.5rem"
      height="3.5rem"
      viewBox="0 0 36 36"
      xmlns="http://www.w3.org/2000/svg"
      //xmlns:xlink="http://www.w3.org/1999/xlink"
      aria-hidden="true"
      role="img"
      //class="iconify iconify--twemoji"
      preserveAspectRatio="xMidYMid meet"
    >
      <path
        fill="#E75A70"
        d="M29 12c-4.688-3.515-6-6-8-6s-2 1-3 1s-1-1-3-1s-3.313 2.485-8 6c-4 3-7 3-7 5s5 6 8 9c2 2 5 4 10 4s8-2 10-4c3-3 8-7 8-9s-3-2-7-5z"
      ></path>
      <path
        fill="#662113"
        d="M28.389 15.5C23.963 14.035 21 13.417 18 13.417s-5.962.618-10.389 2.083C3.833 16.75 1 16.167 1 17c0 .834 9 5 17 5s17-4.166 17-5c0-.833-2.834-.25-6.611-1.5z"
      ></path>
      <path
        fill="#FFF"
        d="M28.389 15.5C23.963 14.035 21 13.417 18 13.417s-5.962.618-10.389 2.083c-3.624 1.199-6.371.715-6.59 1.408c2.454.385 9.119-.644 16.979-.644s14.524 1.029 16.979.644c-.219-.693-2.967-.209-6.59-1.408z"
      ></path>
    </svg>
  );
};

export const ConclusionIcon = () => {
  return (
    <svg
      width="3.5rem"
      height="3.5rem"
      viewBox="0 0 64 64"
      id="fill"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M37,3A23.993,23.993,0,0,0,18.152,41.848L3,57l4,4L22.152,45.848A24,24,0,1,0,37,3Z"
        fill="#e5e6eb"
      />
      <rect
        height="5.657"
        fill="#444"
        transform="translate(-34.755 23.094) rotate(-45)"
        width="15.556"
        x="2.722"
        y="50.672"
      />
      <rect
        height="2.828"
        transform="translate(-35.169 24.094) rotate(-45)"
        width="15.556"
        x="3.722"
        y="53.086"
      />
      <circle cx="37" cy="27" r="20" fill="#d9dae2" />
      <circle cx="37" cy="27" r="19" fill="#cceaff" />
      <rect height="2" fill="#fff" width="27" x="24" y="33" />
      <rect height="8" fill="#027de5" width="4" x="27" y="26" />
      <rect height="14" fill="#027de5" width="4" x="35" y="20" />
      <rect height="16" fill="#027de5" width="4" x="43" y="18" />
      <path
        d="M52.938,9.062A24,24,0,0,1,19.062,42.938,24,24,0,1,0,52.938,9.062Z"
        fill="#cfcfd9"
      />
      <path
        d="M16,48l2,2,4.156-4.156a23.946,23.946,0,0,1-2.12-1.88Z"
        fill="#d9dae2"
      />
      <path d="M37,7A20,20,0,1,0,57,27,20.023,20.023,0,0,0,37,7Zm0,38A18,18,0,1,1,55,27,18.021,18.021,0,0,1,37,45Z" />
      <path d="M37,2A25.028,25.028,0,0,0,12,27a24.715,24.715,0,0,0,4.827,14.758L2.293,56.293a1,1,0,0,0,0,1.414l4,4a1,1,0,0,0,1.414,0L22.241,47.173A24.721,24.721,0,0,0,37,52,25,25,0,0,0,37,2ZM7,59.586,4.414,57,14,47.414,16.586,50ZM37,50a22.754,22.754,0,0,1-14.229-4.938,1,1,0,0,0-1.326.079L18,48.586,15.414,46l3.445-3.445a1,1,0,0,0,.078-1.326A22.757,22.757,0,0,1,14,27,23,23,0,1,1,37,50Z" />
    </svg>
  );
};
